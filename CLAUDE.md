...
- **A teammate's idle notification is not a report.** Their final message does not reach the lead automatically — ask for the full deliverable via `SendMessage` (bare name, no `@session` suffix) or it is lost.
  ...
  **And asking once is not enough, because the failure is on their side of the wire.** On 2026-09-02 all three teammates went idle without delivering, and two rounds of `SendMessage` returned only `"Complete."`. The work existed the whole time. The architect eventually diagnosed it: **it had been answering in plain text, and for an agent-team teammate plain text does not travel — only a `SendMessage` call does.** So a teammate can finish correctly, report correctly by its own lights, and reach nobody.

  **The narrow claim is the true one.** An ordinary subagent — one spawned and awaited inside this session — does return its final message to the caller. The teammates in this table are not that: they are independent sessions reached through a mailbox, and a mailbox only carries what is posted to it. Do not generalise this to "subagent output is discarded"; it is specifically the agent-team channel.

  So: **say the channel in the brief**, not just the task — *reply with `SendMessage`; a plain-text answer will not reach me*. And treat a bare `"Complete."` as worth one more ask rather than as a summary. It is not a proven signature of this failure, but on 2026-09-02 it was what all three sent while holding finished work.

  The cost, as recorded in that session: both the architect and the DBA independently found that `0051`'s premise was wrong, neither reached the lead in time, and the migration and its UI were built, merged, and reverted (`#70`, `#72`, `#74`). Git proves the PRs; **who said what and when is from the session transcript, not from this repository** — so read it as the account it is. **The team was right and the lead never heard it.**
...
**A trap recorded here was itself produced by a trap recorded here, and the retraction is the entry.** On 2026-09-03 this section briefly carried a paragraph saying `scripts/migrate.sh` has no `set -e`, so a missing `psql` let it run to the end, print `done: 0 applied` and exit 0. **Every part of that was false.** `_env.sh:3` runs `set -euo pipefail` in the calling shell, `migrate.sh` sources it before any psql call, and the original aborts with 127 — verified by running it from a path where its globs resolve.

The measurement that produced the false claim was `./scripts/migrate.sh 2>&1 | tail -20`, read as exit 0. **A pipeline's status is the last command's**, which was `tail`. That is the trap described two paragraphs below this one, and it was walked into while writing a paragraph about a different one.

Two things to take from it. **A pipe on the left of a status check invalidates the check**, and knowing that is not the same as remembering it at the moment you are busy proving something else — so make the exit-code read a separate, unpiped command whenever the exit code is the finding. And **a first reproduction that is convenient is not a reproduction**: a later attempt, copying the script to `/tmp`, "confirmed" exit 0 for a third reason entirely — the copy's relative path to `supabase/migrations` no longer resolved, so the loop found nothing and returned before any failure. Two different accidents produced the same wrong answer before the script was finally run where its own paths worked.

What survives is small and real: `psql` is keg-only on this machine, so the bare command is missing from PATH, and `migrate.sh` now says so by name rather than leaving a reader to read `command not found` as a connection problem.
