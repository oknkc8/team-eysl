...
/**
 * The Postgres spelling of Default_Ignorable_Code_Point — 17 ranges, 4174
 * codepoints.
 *
 * Generated by running /\p{Default_Ignorable_Code_Point}/u over every codepoint
 * and compressing to ranges, NOT transcribed. Four rounds of this rule were lost
 * to sets written from memory, each one containing most invisible characters
 * without meaning "invisible". Postgres has no \p{}, so the SQL must enumerate;
 * what it enumerates is a named Unicode property.
 */
const DEFAULT_IGNORABLE_SQL = String.raw`\+0000ad\+00034f\+00061c\+00115f-\+001160\+0017b4-\+0017b5\+00180b-\+00180f\+00200b-\+00200f\+00202a-\+00202e\+002060-\+00206f\+003164\+00fe00-\+00fe0f\+00feff\+00ffa0\+00fff0-\+00fff8\+01bca0-\+01bca3\+01d173-\+01d17a\+0e0000-\+0e0fff`
...
describe('the pattern the browser and the database share', () => {
  ...
  it('asks Postgres for the same character set the browser refuses', () => {
    expect(sql).toContain("v_nickname ~ '[[:space:]]'")
    expect(sql).toContain("or v_nickname ~ '[^[:print:]]'")
    // The Default_Ignorable_Code_Point ranges, generated from the engine rather
    // than typed. Postgres has no \p{}, so the property has to be spelled out —
    // but what is spelled out is a named property, not remembered characters.
    expect(sql).toContain(DEFAULT_IGNORABLE_SQL)
  })
  ...
  it('keeps both halves of the Postgres union', () => {
    ...
    expect(gate).toContain(DEFAULT_IGNORABLE_SQL)
  ...
  it('refuses an applicant who is already on the roster, and does not link them', () => {
    expect(sql).toContain("'reason', 'already_registered'")

    // One reason for BOTH arms — the roster match and the unique violation.
    // Distinguishing them would tell an anonymous caller "this person is on the
    // club roster" as opposed to "this exact nickname is registered", which is
    // the membership oracle in a different costume.
    expect(sql).not.toContain("'reason', 'existing_member'")
    expect(sql).not.toContain("'reason', 'nickname_taken'")
    expect(sql.match(/'reason', 'already_registered'/g)).toHaveLength(2)
  ...
  // `already_registered`, the identical-looking decomposed form returned ok:true.
...
describe('checkNicknameFormat', () => {
  ...
  it('refuses every whitespace, control and invisible-format codepoint', () => {
    const forbidden = [
      ...
      // ROUND 2's MISSES. Bidi controls and isolates are category Cf, so
      // neither [[:cntrl:]] nor \p{Cc} covers them. U+202E is the dangerous
      // one: it does not merely hide, it reverses the display order of
      // everything after it.
      0x61c, 0x200e, 0x200f, 0x202a, 0x202b, 0x202c, 0x202d, 0x202e,
      0x2066, 0x2067, 0x2068, 0x2069, 0xfff9, 0xfffa, 0xfffb,
      // ROUND 3's MISSES — category Mn, which \p{C} and [^[:print:]] both let
      // through. U+034F exists for no purpose except to be an invisible break,
      // and the variation selectors are a whole block of them.
      0x34f, 0xfe00, 0xfe0f, 0x180e, 0x1bca0, 0xe0001, 0xe0020,
      // Hangul-specific, and the reason this matters more here than elsewhere:
      // these render as blank space inside Korean text and are the natural way
      // to forge a lookalike of a Korean nickname.
  ...
  // The cost of using the property, stated as a test so it is a decision rather
  // than a surprise. An emoji carrying a variation selector is refused, because
  // VS16 is precisely the kind of invisible modifier this rule exists to stop;
  // a bare emoji is not. A nickname is 이름/출생년도/성별/지역, so this is a trade
  // worth making — but somebody will hit it, and it should be findable.
  it('refuses an emoji that carries a variation selector, and only for that', () => {
    expect(checkNicknameFormat('창호/98/남/관악😀')).toBeNull()
    expect(checkNicknameFormat('창호/98/남/관악❤️')?.reason).toBe('nickname_invisible')
    // Proof it is the selector and not the heart.
    expect(NICKNAME_FORBIDDEN.test('❤')).toBe(false)
    expect(NICKNAME_FORBIDDEN.test(String.fromCodePoint(0xfe0f))).toBe(true)
  })
